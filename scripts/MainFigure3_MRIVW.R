...
setorder(plotData1,outcomeSource, exposureSetting,-exposure)

#' ## Plot 
p3 = plotData1 |>
  forestplot(labeltext = c(exposure, exposureSetting, outcomeSource, CI, FStat2),
             boxsize = 0.25,
             xticks = seq(from = -1.5, to = 1, by = 0.5),
             xlab = "logHR (95% CI) for BC Survival per 1-SD increment \nin PCSK9 or LDL-C levels (total effects)",
             clip = c(-1.5,1),
             #xlog = TRUE,
             align = c("l","l","l","r","r"),
             vertices = TRUE,
...
filename = paste0("../results/MainFigures/Fig3_MRIVW.png")
