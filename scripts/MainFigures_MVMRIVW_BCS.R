...
plotData1[,lowerCI95 := beta_exp1-1.96*SE_exp1]
plotData1[,upperCI95 := beta_exp1+1.96*SE_exp1]

plotData1[,signif := ifelse(pval_adj1 < 0.05,1,0)]
plotData1[signif==1, y_ast := ceiling(upperCI95)]

plotData2 = copy(plotData1)
setorder(plotData2,outcomeSource,-SNPset,sex,beta_exp1)
plotData2$ES <- paste(rep(" ", 30), collapse = " ")
plotData2$CI <- ifelse(is.na(plotData2$SE_exp1), "",sprintf("%.2f [%.2f, %.2f]",plotData2$beta_exp1, plotData2$lowerCI95, plotData2$upperCI95))
...
filename = paste0("../results/MainFigures/MVMRIVW_BCS_PE.png")
...
filename = paste0("../results/MainFigures/MVMRIVW_BCS_PE_females.png")
