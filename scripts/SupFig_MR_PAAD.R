...
MR_IVW = MR_IVW[grepl("ParentsAgeAtDeath",outcome),]
MR_IVW[, outcome := "Parents' age at death [UKB]"]

MR_ratio[,unique(outcome)]
MR_ratio = MR_ratio[grepl("ParentsAgeAtDeath",outcome),]
MR_ratio = MR_ratio[exposure_pval<0.05,]
MR_ratio[, outcome := "Parents' age at death [UKB]"]
...
MR_merged[,method := c(rep("MR-IVW",8),rep("MR-ratio",11))]
...
MR_merged[,outcome2 := gsub("Parents' age at death","MR-IVW \n",outcome)]
...
MR_merged[signif==1, y_ast:=upperCI95 + 0.01]
# MR_merged[signif==1, y_ast2:=max(upperCI95, na.rm=TRUE) + 0.5]
...
p1 = ggplot(data=MR_merged, aes(x=exposure, y=beta_IVW, ymin=lowerCI95, ymax=upperCI95,color=colGroup)) +
  ...
  ylab("1-SD increment in for Parents' age at death (95% CI) \nper 1-SD increment in PCSK9 levels") + 
...
filename = paste0("../results/Main_Sup_Figs/Sup_ForestPlot_MR_PAAD.png")
