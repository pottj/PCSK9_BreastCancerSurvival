...
plotData1[outcome %in% c("BCS - Mei et al.","BCS - FinnGen_rec"),method := paste(method,"recessive")]

plotData1[outcome == "BCS - BCAC", outcome := "[BCAC]"]
plotData1[outcome == "BCS - FinnGen", outcome := "[FinnGen]"]
...
p1
filename = paste0("../results/MainFigures/Plot1_MRratio_PCSK9.png")
...
#' update outcome and exposure names
plotData2[outcome == "BCS - BCAC", outcome := "[BCAC]"]
plotData2[outcome == "BCS - FinnGen", outcome := "[FinnGen]"]
...
plotData2[,dummy := "PCSK9 level on BC Survival"]
...
p2
filename = paste0("../results/MainFigures/Plot2_MRratio_IVW.png")
...
#' update outcome and exposure names
plotData3[outcome == "BCS - BCAC", outcome := "[BCAC]"]
plotData3[outcome == "BCS - FinnGen", outcome := "[FinnGen]"]
...
plotData3[,dummy := "LDL-C level on BC Survival"]
plotData3[,exposure2 := c(rep(c(1,2,4),each=2),rep(c(1,2,4),each=2),rep(3,4))]
...
p3
filename = paste0("../results/MainFigures/Plot3_MRIVW_LDLC.png")
...
plotData4a[outcome == "BCS - BCAC", outcome := "[BCAC]"]
plotData4a[outcome == "BCS - FinnGen", outcome := "[FinnGen]"]
...
plotData4[,dummy := "PCSK9 levels on BC Survival, cond. on LDL-C"]
...
p4
filename = paste0("../results/MainFigures/Plot4_MVMRIVW_PCSK9.png")
png(filename = filename,width = 2000, height = 1500, res=200)
plot(p4)
dev.off()
...
plotData12[9:10,dummy := "WB PE"]
plotData12[11:12,dummy := "WB PE F"]
plotData12[27:28,dummy := "WB GE"]

plotData12[,dummy := gsub("Brain - Cerebellar Hemisphere","Brain - CH",dummy)]
plotData12[,dummy := gsub("Brain - Cerebellum","Brain - Cb",dummy)]
plotData12[13:14,dummy := "Adipose VO"]
...
p12
filename = paste0("../results/MainFigures/Plot12_PCSK9.png")
png(filename = filename,width = 2400, height = 1800, res=200)
plot(p12)
dev.off()


p12 = ggplot(data=plotData12[c(1:12,15,16,19,20,27,28)], aes(x=dummy, y=beta,
                                  ymin=lowerCI95, ymax=upperCI95,
                                  color=outcome,shape=method,
                                  alpha=sex,fill=outcome)) +
  geom_hline(yintercept=0, lty=2, linewidth =1) +  
...
p12
filename = paste0("../results/MainFigures/Plot12_PCSK9_BCS_filtered.png")
png(filename = filename,width = 2400, height = 900, res=200)
plot(p12)
dev.off()
...
plotData34[signif==1, y_ast:=lowerCI95 - 0.5]
...
p34
filename = paste0("../results/MainFigures/Plot34_LDLC_PCSK9_BCS.png")
png(filename = filename,width = 2800, height = 1200, res=200)
plot(p34)
dev.off()
