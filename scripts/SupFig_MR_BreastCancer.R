...
MR_IVW = MR_IVW[grepl("BreastCancer",outcome),]
MR_IVW[, outcome := "Breast Cancer [FinnGen + UKB]"]

MR_ratio[,unique(outcome)]
MR_ratio = MR_ratio[grepl("BreastCancer",outcome),]
MR_ratio = MR_ratio[exposure_pval<0.05,]
MR_ratio[, outcome := "Breast Cancer [FinnGen + UKB]"]
...
MR_merged[,outcome2 := gsub("Breast Cancer","MR-IVW \n",outcome)]
...
p1 = ggplot(data=MR_merged, aes(x=exposure, y=beta_IVW, ymin=lowerCI95, ymax=upperCI95,color=colGroup)) +
  ...
  ylab("logOR for breast cancer (95% CI) \nper 1-SD increment in PCSK9 levels") + 
...
filename = paste0("../results/Main_Sup_Figs/Sup_ForestPlot_MR_BC.png")
