...
#' # Introduction ####
#' ***
#' LDL-C levels on BCS using rs562556
#' 
#' # Initialize ####
#' ***
rm(list = ls())
...
p2a = plotData2 |>
  ...
  forestplot(labeltext = c(Outcome, geneticModel, maxSamples, sex, CI, FStat2),
             boxsize = 0.25,
             xticks = seq(from = 0, to = 70, by = 10),
             xlab = "logHR for BC Survival (95% CI) per 1-SD increment in LDL-C levels",
             clip = c(-5,70),
             #xlog = TRUE,
             vertices = TRUE,
             txt_gp = fpTxtGp(
...
filename = paste0("../results/SupplementalFigures/MRratio_BCS_LDLC_Mei.png")
...
filename = paste0("../results/SupplementalFigures/MRratio_BCS_LDLC_BCAC.png")
