...
MR_IVW = MR_IVW[grepl("CAD",outcome),]
#MR_IVW[, outcome := "CAD [FinnGen + UKB]"]

MR_ratio[,unique(outcome)]
MR_ratio = MR_ratio[grepl("CAD",outcome),]
MR_ratio = MR_ratio[exposure_pval<0.05,]
#MR_ratio[, outcome := "CAD [FinnGen + UKB]"]
...
MR_merged[,outcome2 := gsub("CAD","MR-IVW \n",outcome)]
...
p1 = ggplot(data=MR_merged, aes(x=exposure, y=beta_IVW, ymin=lowerCI95, ymax=upperCI95,color=colGroup)) +
  ...
  ylab("logOR for coronary atherosclerosis (95% CI) \nper 1-SD increment in PCSK9 levels") + 
...
filename = paste0("../results/Main_Sup_Figs/Sup_ForestPlot_MR_CAD.png")
