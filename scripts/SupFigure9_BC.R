...
#' Filter data 
plotData1 = plotData1[outcome == "Breast Cancer",]
plotData1 = plotData1[!grepl("GE",exposure),]
plotData1[,method := "MR-ratio"]
plotData2 = plotData2[outcome == "Breast Cancer",]
...
p2a = plotData3 |>
  ...
  forestplot(labeltext = c(exposure, exposureSetting, CI,FStat2),
             boxsize = 0.25,
             xticks = seq(from = -2, to = 0.5, by = 0.5),
             xlab = "logOR (95% CI) for BC risk per 1-SD increment in PCSK9 or LDL-C levels",
             #clip = c(-10,100),
             align = c("l","l","r","r"),
             #xlog = TRUE,
             vertices = TRUE,
...
filename = paste0("../results/SupplementalFigures/SupFig9A_MRratio_BC.png")
...
p2b = plotData3 |>
  ...
  forestplot(labeltext = c(exposure, exposureSetting, CI,FStat2),
             boxsize = 0.25,
             xticks = seq(from = -0.2, to = 0.2, by = 0.1),
             xlab = "logOR (95% CI) for BC risk per 1-SD increment in PCSK9 or LDL-C levels",
             #clip = c(-10,100),
             align = c("l","l","r","r"),
             #xlog = TRUE,
...
filename = paste0("../results/SupplementalFigures/SupFig9B_MRIVW_BC.png")
png(filename = filename,width = 2000, height = 600, res=200)
plot(p2b)
dev.off()
