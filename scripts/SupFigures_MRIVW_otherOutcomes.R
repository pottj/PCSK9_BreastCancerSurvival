...
plotData1[grepl("CAD - females",outcome),outcomeSource := paste(outcomeSource,"fem.")]
...
p2a = plotData2 |>
  forestplot(labeltext = c(setting, Outcome, outcomeSource, CI, FStat2),
             boxsize = 0.25,
             xticks = seq(from = -0.1, to = 0.25, by = 0.1),
             xlab = "logOR (95% CI) for BC or CAD Risk per 1-SD increment in PCSK9 GE levels",
             #clip = c(-1.7,1),
             #xlog = TRUE,
             vertices = TRUE,
             txt_gp = fpTxtGp(
...
filename = paste0("../results/SupplementalFigures/MRIVW_otherOutcomes_GE.png")
...
setorder(plotData3,Outcome,gene,setting)

p2b = plotData3 |>
  forestplot(labeltext = c(setting,gene, outcomeSource, CI, FStat2),
             boxsize = 0.25,
             #xticks = seq(from = -2.5, to = 2.5, by = 1),
             xlab = "logOR (95% CI) for BC Survival per 1-SD increment in LDL-C levels",
             #clip = c(-1.7,1),
             #xlog = TRUE,
             vertices = TRUE,
             txt_gp = fpTxtGp(
...
filename = paste0("../results/SupplementalFigures/MRIVW_otherOutcomes_LDLC.png")
png(filename = filename,width = 2200, height = 1400, res=200)
plot(p2b)
dev.off()
